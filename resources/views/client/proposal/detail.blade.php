...
<link rel="stylesheet" href="{{ asset('assets/css/client/proposal/detail.css') }}">
                    ...
                    <div class="p-4 p-sm-5">
                        <div class="d-sm-flex align-items-start gap-3">
                            <div class="flex-shrink-0">
                                <p class="text-muted mb-1"><strong>Proposal By:</strong> {{ $proposal->proposal_submitted_by }}</p>
                        ...
                        <a class="btn btn-outline-primary mb-4"
                            href="{{route('client.job.detail', $job->id)}}" target="_blank">
                        ...
                        <div class="mt-4">
                            <h3 class="h4 fw-semibold mb-4">Messages</h3>
                                @if ($user->role == 'provider')
                                    @dd("provider ".$proposal->provider_id)
                                    <livewire:chat :user_id="$job->client_id" />
                                @elseif ($user->role == 'client')
                                    {{-- @dd("provider is client ",$proposal->provider_id) --}}
                                    {{-- <livewire:chat :user_id="$proposal->provider_id" />  --}}
                                        
                                    {{-- @livewire('chat', ["user_id" => $proposal->provider_id]) --}}
                                    @livewire('chat-component', ["user_id" => $proposal->provider_id, "job_id" => $job->id, "proposal_id" => $proposal->id])
                                @endif
                            
                            {{-- <livewire:chat :user="$job->client_id" />  --}}
                                ...
                                <li class="mb-4">
                                    <div class="d-flex align-items-start">
                                        <div class="me-3 position-relative">
                                            ...
                                            <img src="{{ $profile == null ? asset('assets/images/default.jpg') : asset('storage/assets/profiles/' . $profile->picture) }}"
